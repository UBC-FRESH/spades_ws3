...
#    `functionName` is a function's name in your module (e.g., `harvestingEvent1`).
# 2. Copy this file to the tests folder (i.e., `/media/data/gparadis/dev_/fiim/modules/harvesting/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("harvesting")
  ...
  parameters <- list(
    ...
    harvesting = list(.saveInitialTime = NA)
  )
  ...
  if (exists("harvestingEvent1", envir = .GlobalEnv)) {
    simOutput <- harvestingEvent1(mySim)
  } else {
  ...
  if (exists("harvestingEvent2", envir = .GlobalEnv)) {
    simOutput <- harvestingEvent2(mySim)
  } else {
